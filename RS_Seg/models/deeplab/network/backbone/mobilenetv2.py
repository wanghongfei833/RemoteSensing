...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, output_stride=8, width_mult=1.0, inverted_residual_setting=None,in_channels=4,
                 round_nearest=8):
        """
...
def mobilenet_v2(pretrained=False, progress=True, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['mobilenet_v2'],
                                              progress=progress)
        model_loads = model.state_dict()
        no_grad = []
        for k,v in model_loads.items():
            if k in state_dict.keys() and state_dict[k].size()==v.size():
                model_loads[k] = v
                no_grad.append(k)
            else:
                print(k,'is not load')
        model.load_state_dict(model_loads)
        for name,parm in model.named_children():
            if name in no_grad:
