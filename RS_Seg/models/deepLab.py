...
from RS_Seg.models.backbone import mobilenetv2
...
def _segm_mobilenet(in_channels, num_classes, output_stride, loss_fun=None):
    if output_stride == 8:
        aspp_dilate = [12, 24, 36]
    else:
        aspp_dilate = [6, 12, 18]

    backbone = mobilenetv2.mobilenet_v2(output_stride=output_stride, in_channels=in_channels)
...
def deeplabel_v3p(num_class, in_channels, loss_fun):
    model = _segm_mobilenet(in_channels, num_class, 8, loss_fun)
    model.model_name = "deeplabv3p"
    return model


if __name__ == '__main__':
    md = deeplabel_v3p(2, 3, "bce")
